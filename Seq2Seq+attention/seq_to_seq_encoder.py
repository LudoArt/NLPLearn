...
from data_processing import MakeSrcTrgDataset

# 假设输入数据已经转换成了单词编号的格式
SRC_TRAIN_DATA = "H:/NLPLearn/Seq2Seq+attention/data/train.en"  # 源语言输入文件
TRG_TRAIN_DATA = "H:/NLPLearn/Seq2Seq+attention/data/train.zh"  # 目标语言输入文件
CHECKPOINT_PATH = "H:/NLPLearn/Seq2Seq+attention/data/seq2seq_ckpt"  # checkpoint保存路径
...
class NMTModel(object):
    ...
    def forward(self, src_input, src_size, trg_input, trg_lable, trg_size):
        ...
        # seq2seq模型中不需要用到enc_outputs，而attention模型会用到
        # 此处和之前的代码有所不同（改为双向LSTM）
        with tf.variable_scope("encoder"):
            # 构造编码器时，使用bidirectional_dynamic_rnn构造双向循环网络
            # 双向循环网络的顶层输出enc_outputs是一个包含了两个张量的tuple，每个张量的
        ...
        lable_weights = tf.sequence_mask(trg_size, maxlen=tf.shape(trg_lable)[1], dtype=tf.float32)
        lable_weights = tf.reshape(lable_weights, [-1])
        cost = tf.reduce_sum(loss * lable_weights)
        cost_per_token = cost / tf.reduce_sum(lable_weights)
